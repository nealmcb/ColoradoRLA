...
public final class CastVoteRecordQueries {
  ...
  public static int deleteAll(final Long county_id) {
    final Session s = Persistence.currentSession();
    // CVRContestInfo has a required foreign key to CastVoteRecord so they must be deleted first
    deleteCVRContestInfos(county_id);

    Query query = s.createQuery("delete from CastVoteRecord cvr where cvr.my_county_id = :county_id");
